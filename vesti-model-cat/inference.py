...
def repaint(person, mask, result):
    ...
    person_np = np.array(person)  # Shape: (H, W, 3)
    result_np = np.array(result)  # Shape: (H, W, 3)
    mask_np = np.array(mask) / 255  # Normalize mask to [0, 1], Shape: (H, W)

    # Expand mask dimensions to match person_np and result_np
    mask_np = np.expand_dims(mask_np, axis=-1)  # Shape: (H, W, 1)
    mask_np = np.repeat(mask_np, 3, axis=-1)    # Shape: (H, W, 3)
